...
export default function SlideShow () {
  ...
  const [loadingSlides, setLoading]  = useState(true);
  const [orientation , setOri]       = useState('horizontal');
  const [idx         , setIdx]       = useState(0);
  const [reveal      , setReveal]    = useState(0);
  const [showAll     , setShowAll]   = useState(false);   // paso a paso / completa

  /* ----- mapping de iconos (memo para no recrear) ------------------------- */
  ...
  /* ------------------------------------------------------------------------ */
  /*  1. Cargar presentación + diapositivas                                   */
  useEffect(() => {
    (async () => {
      /* meta presentación */
  ...
  /* ------------------------------------------------------------------------ */
  /*  2. Derivados de estado (NO son hooks)                                   */
  const current      = slides[idx] || {};
  const lines        = (current.content || '').split('\n').filter(l => l.trim());
  const isShort      = lines.length <= 3;
  const Bullet       = current.icon ? iconMap[current.icon] : null;
  const visibleLines = showAll ? lines.length : reveal;

  /* ------------------------------------------------------------------------ */
  /*  3. Navegación + atajos de teclado                                       */
  ...
  /* ------------------------------------------------------------------------ */
  /*  4. Exportar PDF (igual que antes)                                       */
  ...
  /* ------------------------------------------------------------------------ */
  /*  5. Render                                                               */
  if (loadingSlides) return <p>Cargando…</p>;
  ...
  return (
    <>
      ...
      <button onClick={toggle} title="Paso a paso / Completa (M)"
        style={{
          position:'fixed',bottom:'1.2rem',left:'calc(1.2rem + 70px)',zIndex:9999,
          background:'#fff',border:'1px solid #d1d7dc',borderRadius:4,
          padding:'.45rem .6rem',display:'flex',alignItems:'center',gap:'.4rem'
        }}>
      ...
      <button onClick={downloadPDF}
        style={{
          position:'fixed',bottom:'1.2rem',left:'1.2rem',zIndex:9999,
          background:'#fff',border:'1px solid #d1d7dc',borderRadius:4,
          padding:'.45rem .6rem',display:'flex',alignItems:'center',gap:'.4rem'
        }}>
        <FiDownload/> PDF
          ...
          <AnimatePresence mode="wait">
            <motion.div key={current.id} ref={slideRef}
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
                {lines.map((l,i)=>(
                  <div key={i}
                       className={`bullet ${i < visibleLines ? 'shown' : ''} ${i===visibleLines-1?'current':''}`}
              ...
              {current.image_url && (
                <div style={{marginTop:'1rem'}}>
                  <Image src={current.image_url} alt="" width={1000} height={560}
                         style={{maxWidth:'100%',height:'auto'}}/>
                </div>
              )}

              {current.graph_config && visibleLines === lines.length && (
